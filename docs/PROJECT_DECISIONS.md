...
- 결정: 자동 백업 정책, Watchdog, Discord Webhook, 상태 조회 확장, 실제 자동 재시작은 T10 Windows 실기 검증과 로그/증거 수집 이후 2차 범위로 다룬다.
- 근거: 해당 기능들은 서버 로그 형식, Game ID 출력 위치, 안전 종료 방식, 저장 완료 시점, sleep/idle 로그를 확인해야 안전하게 설계할 수 있다.
- 영향: 개발 에이전트 지시 시 T10 전에는 추가 운영 기능 구현을 요구하지 않고, T10 결과 기록과 분석을 우선한다.
